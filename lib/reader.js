...
class Reader {
  ...
  findConfigFileInDir (filename, dir) {
    ...
    if (this.fs.existsSync(target)) {
      ...
      const section = ['openapidoc', 'apidoc'].find(key => foundConfig[key]);
      if (section) {
        this.log.verbose(`Using ${section} key of ${filename}`);
        // pull any missing config from root
        ['version', 'name', 'description'].forEach(key => {
          if (!foundConfig[section][key] && foundConfig[key]) {
            this.log.verbose(`Using ${key} from root of ${filename}`);
            foundConfig[section][key] = foundConfig[key];
          }
        });
        return foundConfig[section];
      }
